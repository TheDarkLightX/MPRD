...
pub fn add(file: PathBuf, name: Option<String>, config_path: Option<PathBuf>) -> Result<()> {
    ...
    // Create storage
    let storage_dir = config.policy_storage.local_dir
        .unwrap_or_else(|| PathBuf::from(".mprd/policies"));
    
    let storage = LocalPolicyStorage::new(&storage_dir)
        .context("Failed to initialize policy storage")?;
    
    // Store policy
    let hash = storage.store(&policy_bytes)
...
pub fn list(format: String, config_path: Option<PathBuf>) -> Result<()> {
    ...
    let hashes = storage.list()
        .context("Failed to list policies")?;
    
    if format == "json" {
        let json_hashes: Vec<String> = hashes.iter()
...
pub fn get(hash_hex: String, format: String, config_path: Option<PathBuf>) -> Result<()> {
    ...
    // Retrieve policy
    let policy_bytes = storage.retrieve(&hash)
...
pub fn validate(file: PathBuf) -> Result<()> {
    ...
    println!("   Size: {} bytes", policy_bytes.len());
    println!("   UTF-8: ✅");
    println!("   Tau-like syntax: {}", if has_tau_keywords { "✅" } else { "⚠️ (no keywords found)" });
    println!();
    
    if !has_tau_keywords {
        println!("⚠️  Warning: No Tau keywords detected. This may not be a valid Tau specification.");
