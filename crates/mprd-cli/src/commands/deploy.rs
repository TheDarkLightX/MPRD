...
use mprd_risc0_shared::{
    ...
    policy_exec_kind_tau_compiled_id_v1, policy_exec_version_id_v1, tau_compiled_policy_hash_v1,
    Id32,
};
...
use mprd_zk::registry_state::{
    AuthorizedPolicySourceBoundaryV1, AuthorizedPolicySourceGovernanceV1,
    AuthorizedPolicySourceIntentV1, AuthorizedPolicyV1, RegistryStateProvider, RegistryStateV1,
...
impl PolicySourceGovernanceSummary {
    fn observe(&mut self, authorized_policy: &AuthorizedPolicyV1) {
        match authorized_policy.source_governance() {
            AuthorizedPolicySourceGovernanceV1::TauGovernedMapped => {
                self.tau_governed_mapped += 1;
            }
            AuthorizedPolicySourceGovernanceV1::OtherMapped => {
                self.other_mapped += 1;
            }
            AuthorizedPolicySourceGovernanceV1::Unmapped => {
                self.unmapped += 1;
            }
        }
    }
...
fn source_governance_label(source_governance: AuthorizedPolicySourceGovernanceV1) -> &'static str {
    match source_governance {
        AuthorizedPolicySourceGovernanceV1::TauGovernedMapped => "TauGovernedMapped",
        AuthorizedPolicySourceGovernanceV1::OtherMapped => "OtherMapped",
        AuthorizedPolicySourceGovernanceV1::Unmapped => "Unmapped",
    }
}

fn source_intent_classification_label(
    source_intent: AuthorizedPolicySourceIntentV1,
) -> &'static str {
    match source_intent {
        AuthorizedPolicySourceIntentV1::TauDeclared => "TauDeclared",
        AuthorizedPolicySourceIntentV1::OtherDeclared => "OtherDeclared",
        AuthorizedPolicySourceIntentV1::Undeclared => "Undeclared",
    }
}

fn source_boundary_classification_label(
    source_boundary: AuthorizedPolicySourceBoundaryV1,
) -> &'static str {
    match source_boundary {
        AuthorizedPolicySourceBoundaryV1::TauGovernedMapped => "TauGovernedMapped",
        AuthorizedPolicySourceBoundaryV1::OtherMapped => "OtherMapped",
        AuthorizedPolicySourceBoundaryV1::TauCompiledCarrierUnmapped => {
            "TauCompiledCarrierUnmapped"
        }
        AuthorizedPolicySourceBoundaryV1::OtherCarrierUnmapped => "OtherCarrierUnmapped",
    }
}
...
fn build_bundle_policy_report(
    ...
    BundlePolicyReport {
        ...
        source_governance: source_governance_label(authorized_policy.source_governance()).into(),
        source_intent_classification: source_intent_classification_label(
            authorized_policy.source_intent_classification(),
        )
        .into(),
        source_boundary_classification: source_boundary_classification_label(
            authorized_policy.source_boundary_classification(),
        )
        .into(),
...
fn source_artifact_witness_state_label(
    ...
    if !authorized_policy.policy_source_mapping_present() {
...
fn build_production_release_report(
    ...
    for authorized_policy in &state.authorized_policies {
        ...
        policies.push(ProductionReleasePolicyReport {
            ...
            source_governance: source_governance_label(authorized_policy.source_governance())
                .into(),
            artifact_validation: artifact_validation.into(),
        });
    }
...
mod tests {
    ...
    fn run_production_release_boundary_case(
        ...
        let mut manifest = match mutation {
        ...
        if mutation == ProductionReleaseBoundaryMutation::AllZeroManifestImageId {
            manifest.entries[0].image_id = [0u8; 32];
        }
        ...
        let state = RegistryStateV1 {
            ...
            authorized_policies: vec![AuthorizedPolicyV1 {
                ...
                policy_source_intent_kind_id: None,
            }],
        ...
        let registry_state_path =
            if mutation == ProductionReleaseBoundaryMutation::AllZeroManifestImageId {
                let signed = SignedRegistryStateV1 {
                    registry_state_version: mprd_zk::registry_state::REGISTRY_STATE_VERSION,
                    state,
                    signed_at_ms: 456,
                    signer_pubkey: registry_signer.verifying_key().to_bytes(),
                    signature: Vec::new(),
                };
                let registry_state_path = bundle_dir.join("registry_state.json");
                write_file(
                ...
                write_signed_registry_state(&bundle_dir, &registry_signer, state)
            };
    ...
    fn build_bundle_policy_report_distinguishes_tau_compiled_unmapped_boundary() {
        let policy = AuthorizedPolicyV1 {
            ...
            policy_source_kind_id: None,
            policy_source_hash: None,
            policy_source_intent_kind_id: Some(policy_source_kind_tau_id_v1()),
        };
    ...
    fn production_release_rejects_all_zero_image_id() {
        ...
        let mut manifest = GuestImageManifestV1::sign(
            &manifest_signer,
            123,
            vec![GuestImageEntryV1 {
                policy_exec_kind_id: policy_exec_kind_mpb_id_v1(),
        ...
        assert!(
            err_chain.contains("manifest image_id must not be all-zero"),
