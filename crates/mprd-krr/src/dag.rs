...
use crate::types::{FactId, RuleFingerprint, JustificationHash};
...
impl Justification {
    ...
    pub fn derived(fact: FactId, deps: Vec<FactId>, dep_hashes: &[JustificationHash]) -> Self {
        ...
        Justification { fact, rule, deps, hash }
    }
    ...
    pub fn axiom(fact: FactId) -> Self {
        ...
        Justification { fact, rule, deps: Vec::new(), hash }
    }
...
mod tests {
    ...
    fn test_derived_justification() {
        ...
        let just_c = Justification::derived(
            c,
            vec![a, b],
            &[just_a.hash, just_b.hash],
        );
