...
pub fn explain_why(
    ...
    let fact_name = fact_names.get(fact_id).cloned().unwrap_or_else(|| format!("{}", fact_id));
...
fn explain_justification(
    ...
    let fact_name = fact_names.get(&just.fact).cloned().unwrap_or_else(|| format!("{}", just.fact));
    ...
    if just.is_axiom() {
        ...
        let antecedents = if depth > 0 {
            ...
            just.deps.iter()
                .map(|dep| {
                    let name = fact_names.get(dep).cloned().unwrap_or_else(|| format!("{}", dep));
                    let t = trust.get(dep).map(|t| t.value()).unwrap_or(1.0);
                    DerivationExplanation::Axiom { fact: name, source: None, trust: t }
                })
                .collect()
        };
...
fn render_derivation(d: &DerivationExplanation, output: &mut String, indent: usize) {
    ...
    match d {
        DerivationExplanation::Axiom { fact, source, trust } => {
            if let Some(src) = source {
                output.push_str(&format!("{}{} is an axiom from {} (trust {:.2})\n", prefix, fact, src, trust));
            } else {
                output.push_str(&format!("{}{} is an axiom (trust {:.2})\n", prefix, fact, trust));
            }
        }
        DerivationExplanation::Derived { fact, antecedents, rule, trust } => {
            let rule_str = rule.as_deref().unwrap_or("rule");
            let ant_names: Vec<_> = antecedents.iter().map(|a| match a {
