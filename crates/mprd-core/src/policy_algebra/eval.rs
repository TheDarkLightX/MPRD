...
pub fn evaluate(
    ...
    fn has_missing_signal(expr: &PolicyExpr, ctx: &impl EvalContext) -> bool {
        match expr {
            PolicyExpr::Atom(a) | PolicyExpr::DenyIf(a) => ctx.signal(a).is_none(),
            PolicyExpr::Not(p) => has_missing_signal(p, ctx),
            PolicyExpr::All(children) | PolicyExpr::Any(children) => {
                children.iter().any(|ch| has_missing_signal(ch, ctx))
            }
            PolicyExpr::Threshold { children, .. } => {
                children.iter().any(|ch| has_missing_signal(ch, ctx))
            }
    ...
    fn eval_node(
        ...
        let out = match expr {
            ...
            PolicyExpr::Not(p) => {
                ...
                let out = if has_missing_signal(p, ctx) {
