...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        ...
        Command::FinalizeEpoch => {
            ...
            let next = State {
                epoch: if (state.epoch < 100) { (state.epoch.checked_add(1).ok_or(Error::Overflow)?) } else { state.epoch },
                epoch_budget: state.epoch_budget.clone(),
                phase: Phase::Paid,
                recipients_paid: state.recipients_paid.clone(),
                total_payout: state.total_payout.clone(),
        ...
        Command::PayRecipient { amt } => {
            ...
            let guard_ok = ((state.recipients_paid < 10) && (post_total_payout <= 1000) && (post_total_payout <= state.epoch_budget) && (Phase::Approved == state.phase));
            if !guard_ok {
            ...
            let next = State {
                ...
                recipients_paid: (state.recipients_paid.checked_add(1).ok_or(Error::Overflow)?),
