//! Step function for mprd_work_verification.
...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        ...
        Command::Reject => {
            let guard_ok = (Phase::Proofchecking == state.phase)
                && (VerifierResult::Verifierinvalid == state.verifier_result)
        ...
        Command::Reward => {
            let guard_ok = (1 == state.proof_valid)
                && (1 == state.spec_satisfied)
            ...
            let next = State {
                ...
                verifier_result: if state.work_hash > 990 {
                    ...
                    if (state.work_hash <= 800) && (state.work_hash > 0) {
                        ...
                        VerifierResult::Verifierinvalid
                    }
                },
                work_hash: state.work_hash.clone(),
            };
