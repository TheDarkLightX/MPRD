...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        ...
        Command::PayMiner => {
            let mul_result_1 = (state.rewards_paid.checked_add(1).ok_or(Error::Overflow)?).checked_mul(10).ok_or(Error::Overflow)?;

            let guard_ok = (state.rewards_paid < state.valid_count) && (mul_result_1 <= state.round_reward) && (Phase::Evaluated == state.phase);
            if !guard_ok {
            ...
            let next = State {
                dispute_filed: state.dispute_filed.clone(),
                phase: if (state.rewards_paid.checked_add(1).ok_or(Error::Overflow)?) == state.valid_count { Phase::Paid } else { Phase::Evaluated },
                proof_valid: state.proof_valid.clone(),
        ...
        Command::RunProofCheck => {
            ...
            let next = State {
                ...
                submissions_count: state.submissions_count.clone(),
                valid_count: state.valid_count.clone(),
                verifier_result: if state.work_hash > 990 { VerifierResult::Inconclusive } else { if (state.work_hash <= 800) && (state.work_hash > 0) { VerifierResult::Inconclusive } else { VerifierResult::Invalid } },
                work_hash: state.work_hash.clone(),
            };
            let post = next;
            check_invariants(&post)?;
        ...
        Command::RunSpecCheck => {
            ...
            let next = State {
                dispute_filed: state.dispute_filed.clone(),
                phase: if state.work_hash > 990 { Phase::Proofchecking } else { Phase::Evaluated },
                proof_valid: state.proof_valid.clone(),
                rewards_paid: 0,
                round_reward: state.round_reward.clone(),
                spec_satisfied: if state.work_hash > 990 { 0 } else { if (state.work_hash <= 900) && (state.work_hash > 0) { 1 } else { 0 } },
                submissions_count: state.submissions_count.clone(),
                valid_count: if (1 == state.proof_valid) && ((state.work_hash <= 900) && (state.work_hash > 0)) { state.submissions_count } else { 0 },
                verifier_result: if state.work_hash > 990 { VerifierResult::Inconclusive } else { if (1 == state.proof_valid) && ((state.work_hash <= 900) && (state.work_hash > 0)) { VerifierResult::Valid } else { VerifierResult::Invalid } },
                work_hash: state.work_hash.clone(),
            };
            let post = next;
            check_invariants(&post)?;
        ...
        Command::Submit => {
            ...
            let next = State {
                ...
                round_reward: state.round_reward.clone(),
                spec_satisfied: state.spec_satisfied.clone(),
                submissions_count: (state.submissions_count.checked_add(1).ok_or(Error::Overflow)?),
                valid_count: state.valid_count.clone(),
                verifier_result: state.verifier_result.clone(),
                work_hash: state.work_hash.clone(),
            };
