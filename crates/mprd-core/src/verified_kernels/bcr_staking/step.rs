...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        Command::Bond { amt } => {
            ...
            let guard_ok = ((false == state.pending_slash)
                && (0 == state.bonded_amount)
        ...
        Command::Slash => {
            let guard_ok = ((Phase::Idle != state.phase)
                && (Phase::Slashed != state.phase)
                && (state.bonded_amount > 0));
            if !guard_ok {
                return Err(Error::PreconditionFailed("slash guard"));
            }

            let next = State {
                bonded_amount: if state.pending_slash {
                    0
                } else {
                    state.bonded_amount
                },
                pending_slash: if state.pending_slash { false } else { true },
                phase: if state.pending_slash {
                    Phase::Slashed
                } else {
                    state.phase
                },
