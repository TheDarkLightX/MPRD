...
pub fn check_invariants(state: &State) -> Result<(), Error> {
    ...
    if !(((!((ModelResult::Accepted == state.result) && state.checkpoint_required)) || state.checkpoint_ok)) {
        return Err(Error::InvariantViolation("AcceptRequiresCheckpoint"));
    }

    // AcceptRequiresMST
    if !(((!(ModelResult::Accepted == state.result)) || state.mst_consistency_ok)) {
        return Err(Error::InvariantViolation("AcceptRequiresMST"));
    }

    // AcceptRequiresSig
    if !(((!(ModelResult::Accepted == state.result)) || state.commit_sig_ok)) {
