...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        Command::AdvanceEpoch => {
            ...
            let next = State {
                emission_rate: if (state.epoch.checked_add(1).ok_or(Error::Overflow)?) < state.halving_period { 1000 } else { if (state.epoch.checked_add(1).ok_or(Error::Overflow)?) < (state.halving_period.checked_mul(2).ok_or(Error::Overflow)?) { 500 } else { if (state.epoch.checked_add(1).ok_or(Error::Overflow)?) < (state.halving_period.checked_mul(3).ok_or(Error::Overflow)?) { 250 } else { if (state.epoch.checked_add(1).ok_or(Error::Overflow)?) < (state.halving_period.checked_mul(4).ok_or(Error::Overflow)?) { 125 } else { 100 } } } },
        ...
        Command::EmitTokens { amt } => {
            ...
            let post_total_emitted = amt.checked_add(state.total_emitted).ok_or(Error::Overflow)?;

            let guard_ok = (post_epoch_budget <= state.emission_rate) && (post_total_emitted <= 10000);
            if !guard_ok {
            ...
            let next = State {
                ...
                halving_period: state.halving_period.clone(),
                total_emitted: (amt.checked_add(state.total_emitted).ok_or(Error::Overflow)?),
            };
            let post = next;
            check_invariants(&post)?;
            let effects = Effects::default();
        ...
        Command::HalveRate => {
            let mul_result_1 = ({ let n = state.epoch; let d = state.halving_period; if d == 0 { 0 } else { n.div_euclid(d) } }).checked_mul(state.halving_period).ok_or(Error::Overflow)?;
