...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        Command::AdvanceWindow => {
            if !(((state.current_time.checked_sub(state.window_start).ok_or(Error::Underflow)?) >= state.window_size)) {
            ...
            let next = State {
                ...
                window_start: (state.window_size.checked_add(state.window_start).ok_or(Error::Overflow)?),
            };
            let mut post = next;
        ...
        Command::ConsumeNonce { nonce_time } => {
            ...
            let guard_ok = ((state.consumed_count < 1000) && (nonce_time <= state.current_time) && (nonce_time >= state.window_start));
            if !guard_ok {
