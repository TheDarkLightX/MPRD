...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        Command::Challenge => {
            let guard_ok = ((Phase::Pending == state.phase) || ((state.round_count < 3) && (Phase::Challenged == state.phase)));
            if !guard_ok {
            ...
            let next = State {
                ...
                round_count: if (Phase::Pending == state.phase) { 0 } else { (state.round_count.checked_add(1).ok_or(Error::Overflow)?) },
            };
            let mut post = next;
        ...
        Command::Finalize => {
            ...
            let next = State {
                has_verdict: if (Phase::Resolved == state.phase) { true } else { false },
