...
pub fn check_invariants(state: &State) -> Result<(), Error> {
    ...
    // Soundness
    if !((((!(Phase::Challenged == state.phase)) || (false == state.has_verdict)) && ((!(Phase::Pending == state.phase)) || (false == state.has_verdict)) && ((!(Phase::Resolved == state.phase)) || (true == state.has_verdict)) && ((!(Phase::Slashed == state.phase)) || (true == state.has_verdict)))) {
