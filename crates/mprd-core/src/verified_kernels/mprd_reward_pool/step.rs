...
pub fn step(state: &State, cmd: Command) -> Result<(State, Effects), Error> {
    ...
    let (post, effects) = match cmd {
        Command::AddToPool { amt } => {
            ...
            let next = State {
                distributed_total: 0,
                phase: if Phase::Empty == state.phase {
                    Phase::Accumulating
                } else {
                    state.phase
                },
        ...
        Command::PayRecipient { amt } => {
            ...
            let post_distributed_total = amt
                .checked_add(state.distributed_total)
                .ok_or(Error::Overflow)?;

            let guard_ok = (post_distributed_total <= state.pool_balance_at_distribution_start)
                && (amt <= state.pool_balance)
                && (Phase::Distributing == state.phase)
                && (state.recipients_count > 0)
                && ((state.recipients_count > 1)
                    || ((1 == state.recipients_count) && (amt == state.pool_balance)));
            if !guard_ok {
                return Err(Error::PreconditionFailed("pay_recipient guard"));
            }

            let next = State {
                distributed_total: if 1 == state.recipients_count {
                    0
                } else {
                    amt.checked_add(state.distributed_total)
                        .ok_or(Error::Overflow)?
                },
                phase: if 1 == state.recipients_count {
                    Phase::Empty
                } else {
                    Phase::Distributing
                },
                pool_balance: if 1 == state.recipients_count {
                    0
                } else {
                    state
                        .pool_balance
                ...
                pool_balance_at_distribution_start: if 1 == state.recipients_count {
                    0
                } else {
                    state.pool_balance_at_distribution_start
                },
                recipients_count: if 1 == state.recipients_count {
                    0
                } else {
                    state
                        .recipients_count
        ...
        Command::StartDistribution => {
            let guard_ok = (Phase::Accumulating == state.phase)
                && (state.pool_balance > 0)
                && (state.recipients_count > 0);
            if !guard_ok {
