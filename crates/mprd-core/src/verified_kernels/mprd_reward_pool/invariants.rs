...
pub fn check_invariants(state: &State) -> Result<(), Error> {
    ...
    if state.pool_balance_at_distribution_start < 0u64
        || state.pool_balance_at_distribution_start > 1000u64
    {
    ...
    if !((!(Phase::Distributing == state.phase))
        || ((state
            .distributed_total
            .checked_add(state.pool_balance)
    ...
    if !((!(Phase::Distributing != state.phase))
        || ((0 == state.distributed_total) && (0 == state.pool_balance_at_distribution_start)))
    {
