...
pub fn check_invariants(state: &State) -> Result<(), Error> {
    ...
    if !(((!(ModelResult::Executed == state.result)) || (state.action_hash_matches && state.limits_binding_ok && state.preimage_present && state.schema_valid))) {
        return Err(Error::InvariantViolation("ExecuteRequiresAllBindings"));
    }

    // RejectedImpliesBindingFailed
    if !(((!(ModelResult::Rejected == state.result)) || (!(state.action_hash_matches && state.limits_binding_ok && state.preimage_present && state.schema_valid)))) {
