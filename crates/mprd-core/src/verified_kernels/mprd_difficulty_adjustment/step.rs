    ...
    let (post, effects) = match cmd {
        Command::AdjustDown => {
            ...
            let next = State {
                ...
                difficulty_level: std::cmp::max(
                    1,
                    std::cmp::min(100, {
                        let n = state
                            .adjustment_factor
                            .checked_mul(state.difficulty_level)
        ...
        Command::EndWindow => {
            ...
            let next = State {
                adjustment_factor: if state.blocks_in_window > state.target_rate {
                    std::cmp::min(
                        200,
                        ((state
                            .blocks_in_window
                            .checked_sub(state.target_rate)
                    ...
                    if state.blocks_in_window < state.target_rate {
                        std::cmp::max(
                            0,
                            100u64
                                .checked_sub(
                                    (state
                                        .target_rate
                                        .checked_sub(state.blocks_in_window)
                                        .ok_or(Error::Underflow)?)
                                    .checked_mul(10)
                                    .ok_or(Error::Overflow)?,
                                )
