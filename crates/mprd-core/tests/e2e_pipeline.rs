...
use mprd_core::{
    CandidateAction, Decision, DecisionToken, DefaultSelector, ExecutionResult,
    ExecutorAdapter, Hash32, PolicyEngine, PolicyHash, ProofBundle, Result,
    RuleVerdict, Score, Selector, StateSnapshot, Value, VerificationStatus,
...
use mprd_core::components::{
    ...
    CryptoDecisionTokenFactory, SignatureVerifyingExecutor,
};
...
impl PolicyEngine for AllowAllPolicyEngine {
    fn evaluate(
        ...
        Ok(candidates.iter().map(|_| RuleVerdict {
            allowed: true,
            reasons: vec![],
            limits: HashMap::new(),
        }).collect())
...
impl PolicyEngine for RiskThresholdPolicyEngine {
    fn evaluate(
        ...
        Ok(candidates.iter().map(|c| {
            ...
            RuleVerdict {
                allowed,
                reasons: if allowed { vec![] } else { vec!["risk too high".into()] },
                limits: HashMap::new(),
            }
        }).collect())
    }
...
fn e2e_basic_pipeline_selects_highest_score() {
    ...
    // Evaluate
    let verdicts = policy_engine.evaluate(&policy_hash, &state, &candidates).unwrap();
...
fn e2e_full_pipeline_with_signature_verification() {
    ...
    let state = StateSnapshot {
        fields: HashMap::from([
            ("user_id".into(), Value::String("alice".into())),
        ]),
        policy_inputs: HashMap::new(),
