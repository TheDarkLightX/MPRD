...
fn bench_proving_time(c: &mut Criterion) {
    ...
    for num_ops in [10, 100, 1000].iter() {
        ...
        // Custom MPB proof
        group.bench_with_input(BenchmarkId::new("mpb_custom", num_ops), num_ops, |b, _| {
            b.iter(|| {
                ...
                if let TracingResult::Success { trace, .. } = result {
                    ...
                    let _proof = prover.prove(&trace).expect("prove");
                }
            });
        });
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    simulate_risc0_proving(black_box(&bytecode), black_box(&registers), *num_ops);
                });
            },
        );
    }
...
fn bench_verification_time(c: &mut Criterion) {
    ...
    for num_ops in [10, 100, 1000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("mpb_verify", num_ops), num_ops, |b, _| {
            b.iter(|| {
...
fn bench_proof_size(c: &mut Criterion) {
    ...
    for num_ops in [10, 100, 1000, 4000].iter() {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let _proof = prover.prove(black_box(&trace)).expect("prove");
                });
            },
        );
    }
