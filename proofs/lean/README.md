...
- `MPRD_SignedRegistryServeAttestationHashRefinement.lean`: a lightweight top-level refinement bridge for that richer signed-registry attestation-hash serve model: once those exact hash-bound serve facts hold on an executed state, the state now composes through the grouped generic execute-ready artifact lane and then into a reachable abstract `MPRD_ExecutionBoundary` state without an extra witness premise at that top-level model.
