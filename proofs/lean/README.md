...
- `MPRD_ExecutionReadyArtifactRuntimeRefinement.lean`: a lightweight grouped-artifact compiler/refinement bridge for the generic execute-ready lane: once the live execute-ready packet admissions and grouped binding/refinement/authorization packet family exist together, there is one canonical grouped runtime witness for the next proof step, and executed grouped-packet states refine through that grouped artifact language instead of a hand-built runtime witness.
...
- `MPRD_RegistryGovernanceExecutionAuthorizationPacketCompiler.lean`: a lightweight local compiler theorem for the grouped concrete ready-bridge authorization packet: once the concrete governance sources reconcile, the live execution-authorization witness projected into grouped metadata is extensionally equal to the direct concrete registry/governance metadata compiler, and the grouped packet preserves both the witness and grouped metadata exactly.
- `MPRD_RegistryGovernanceExecutionAuthorizationArtifactCompiler.lean`: a lightweight local compiler theorem for the next concrete ready-bridge seam: once the live grouped registry/governance authorization packet exists together with exact boundary, executor-admission, binding-vector, and refinement packets, it compiles to one canonical signed-registry execution artifact and that artifact also admits the smaller generic execute-ready artifact witness, instead of rebuilding authorization metadata and exact witness bits by hand.
...
- `MPRD_SignedRegistryExecutionArtifactRuntimeRefinement.lean`: a lightweight grouped-artifact compiler/refinement bridge for the signed-registry runtime lane: once the shipped grouped execution artifact exists and its grouped `ExecutionReadyPacketV1` admissions hold, there is one canonical grouped runtime witness for the next proof step, and the same signed-registry artifact also admits a witness in the smaller generic execute-ready artifact language, so executed grouped `ExecutionReadyPacketV1` states refine through that artifact lane without manually reconstructing either an exact packet or a hand-built runtime witness.
...
- `MPRD_SignedRegistryServeEndToEndRefinement.lean`: a lightweight top-level composed refinement bridge for the shipped signed-registry `mprd serve` path: executed serve states refine through the grouped local `ExecutionReadyPacketV1` boundary and then into the abstract `MPRD_ExecutionBoundary` theorem, and the proof now derives both its generic execute-ready artifact and its runtime witness from the shared concrete registry/governance authorization-artifact compiler route instead of carrying parallel hand-built artifact or witness paths.
...
- `MPRD_SignedRegistryServeAttestationHashRefinement.lean`: a lightweight top-level refinement bridge for that richer signed-registry attestation-hash serve model: once those exact hash-bound serve facts hold on an executed state, the state now discharges its generic execute-ready artifact witness through the concrete registry/governance authorization-packet artifact compiler and then reaches a reachable abstract `MPRD_ExecutionBoundary` state without an extra witness premise at that top-level model.
- `MPRD_SignedRegistryServeAttestationHashArtifactBoundary.lean`: a lightweight top-level boundary theorem package for the stronger attestation-hash-plus-ready-packet serve model: once those richer signed-registry serve facts and exact grouped ready-packet admissions hold on an executed state, the state must first materialize a grouped signed-registry execution artifact through the grouped concrete registry/governance authorization-packet artifact compiler, and it also has a named executed ready-packet view theorem, before the exact signed-registry packet lane can be used.
- `MPRD_SignedRegistryServeAttestationHashExactPacketRefinement.lean`: a lightweight top-level refinement bridge for the stronger attestation-hash-plus-ready-packet serve model: once those richer signed-registry serve facts and exact grouped ready-packet admissions hold on an executed state, the state composes directly through the grouped signed-registry execution artifact lane and then reaches a reachable abstract `MPRD_ExecutionBoundary` state without an extra witness premise at that top-level model.
- `MPRD_SignedRegistryServeAttestationHashIdempotentFileComposition.lean`: a lightweight concrete composition theorem for the strongest shipped local-effect lane: when the attestation-hash-strengthened signed-registry serve path and the shipped `idempotent_file` sink are modeling the same successful execution, the combined state requires both the stronger serve attestation-hash facts and the concrete durable local-file barrier facts.
...
- `MPRD_SignedRegistryServeAttestationHashReadyPacketIdempotentFileRefinement.lean`: a lightweight top-level refinement theorem for the strongest shipped local-effect lane on the richer ready-packet serve model: when that stronger serve model and the shipped `idempotent_file` sink are modeling the same executed path, the combined state reuses the shared top-level attestation-hash exact-packet refinement route to obtain the abstract `MPRD_ExecutionBoundary` witness and also requires the concrete durable local-file barrier facts.
- `MPRD_SignedRegistryServeAttestationHashReadyPacketIdempotentHttpRefinement.lean`: a lightweight top-level refinement theorem for the strongest shipped remote-effect lane on the richer ready-packet serve model: when that stronger serve model and the shipped `idempotent_http` sink are modeling the same successful execution, the combined state reuses the shared top-level attestation-hash exact-packet refinement route to obtain the abstract `MPRD_ExecutionBoundary` witness and also requires the concrete pending/committed HTTP barrier facts.
...
lake env lean MPRD_ExecutionReadyArtifactRuntimeRefinement.lean
...
lake env lean MPRD_RegistryGovernanceExecutionAuthorizationPacketCompiler.lean
lake env lean MPRD_RegistryGovernanceExecutionAuthorizationArtifactCompiler.lean
...
lake env lean MPRD_SignedRegistryServeAttestationHashIdempotentFileComposition.lean
lake env lean MPRD_SignedRegistryServeAttestationHashIdempotentHttpComposition.lean
lake env lean MPRD_SignedRegistryServeAttestationHashReadyPacketIdempotentFileRefinement.lean
lake env lean MPRD_SignedRegistryServeAttestationHashReadyPacketIdempotentHttpRefinement.lean
