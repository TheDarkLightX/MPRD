...
- `MPRD_SignedRegistryServeEndToEndRefinement.lean`: a lightweight top-level composed refinement bridge for the shipped signed-registry `mprd serve` path: executed serve states refine through the grouped local `ExecutionReadyPacketV1` boundary and then into the abstract `MPRD_ExecutionBoundary` theorem, and the proof now composes through the grouped generic execute-ready artifact lane with its runtime witness compiled from that shared artifact instead of carrying a parallel hand-built witness path.
