...
- Runtime refinement still stops short of one top-level concrete execution theorem.
  ...
  - Remaining gap: the witness-gated top-level refinement theorem now exists, but the refinement witness is still an extra premise rather than something fully discharged from concrete runtime objects by one shipped proof chain.
