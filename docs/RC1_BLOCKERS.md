...
- Runtime refinement still stops short of one top-level concrete execution theorem.
  ...
  - Narrowed further: the shipped runtime now also records a deterministic `execution_boundary_refinement_hash` over the exact grouped ready packet plus the verified attestation metadata that reached `execute_ready(...)`, so the concrete refinement artifact is no longer only reconstructible from multiple local surfaces.
