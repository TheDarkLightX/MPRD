...
- Tau source to compiled artifact semantic equivalence is still open.
  - Narrowed: the shipped curated Policy Algebra menu family now has a tracked deterministic receipt at `docs/receipts/rc1_policy_menu_tau_equivalence_20260326.json`, generated by `tools/policy/replay_menu_tau_equivalence_rc1.py` and checked in `.github/workflows/policy-menu-tau-equivalence.yml`, so the built-in menu entries are no longer only “expected” to match their emitted Tau gates.
