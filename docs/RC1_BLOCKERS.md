...
- Governance-to-execution closure is still incomplete.
  ...
  - Narrowed further: proofs built from a signed registry checkpoint now also carry a deterministic checkpoint-attestation hash, and the signed-registry ready bridge re-checks that exact checkpoint binding fail-closed before rebuilding the live ready bundle.
