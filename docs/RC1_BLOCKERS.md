...
- Governance-to-execution closure is still incomplete.
  ...
  - Narrowed further: the same trustless/private `mprd serve` path now fails fast at startup if the registry or signed-state trust anchors are missing, malformed, or signature-invalid, instead of only surfacing the misconfiguration after the process is already up.
