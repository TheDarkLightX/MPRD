...
\begin{center}
\texttt{proofs/lean/CEO\_SimplexPOR.lean}
\end{center}
The proof bundle builds with:
\begin{center}
\texttt{cd proofs/lean \&\& lake build}
\end{center}
...
\begin{lstlisting}[language=lean,caption={Reachability completeness under canonicalizeSwap (Lean)}]
...
\textbf{symmetry quotienting} win-rate \(\ge 0.75\) and median runtime ratio \(\le 1.0\),
implemented by \texttt{tools/ceo/check\_ceo\_simplex\_sweep\_strict.py --gate sym}.
