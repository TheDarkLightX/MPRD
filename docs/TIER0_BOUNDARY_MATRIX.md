...
| Governance admission witness | zero-hot lane; non-one-hot lane; whitespace-padded booleans; malformed prepared governance packet; concrete `GovernanceGateInput` must admit or reject identically to the core state-rail constructor | [lib.rs](../crates/mprd-core/src/lib.rs), [decentralization.rs](../crates/mprd-zk/src/decentralization.rs) | no dedicated fuzz target yet; runtime refinement from concrete governance state and registry-governance objects into the live execution authorization packet is still open |
