...
| Anti-replay boundary | duplicate nonces; claimed-vs-used transitions; high-trust vs low-trust sequencing; failure must not consume nonce | [anti_replay.rs](../crates/mprd-core/src/anti_replay.rs), [e2e_pipeline.rs](../crates/mprd-core/tests/e2e_pipeline.rs), [anti_replay_state_machine.rs](../fuzz/fuzz_targets/anti_replay_state_machine.rs), [PARALLELIZATION_AND_NETWORK_RESILIENCE.md](PARALLELIZATION_AND_NETWORK_RESILIENCE.md) | global at-most-once execution across crash/partition scenarios still needs tighter runtime/deployment closure |
| Governance admission witness | zero-hot lane; non-one-hot lane; whitespace-padded booleans; malformed prepared governance packet | [lib.rs](../crates/mprd-core/src/lib.rs) | no dedicated fuzz target yet; runtime refinement from concrete governance state into execution-time `governance_ok` is still open |
